...
class PointCloud(ABC):
    ...
    def from_file_multisweep(cls,
        ...
        for _ in range(nsweeps):
            ...
            # compensate for Doppler effect in each radar scan
            beta = 0.04
            if comp:
                for i in range(current_pc.points.shape[1]):
                    pt = current_pc.points[:2,i]
                    pt_norm = pt / np.linalg.norm(pt)
                    pt_v = current_pc.points[6:8, i]
                    pt = pt - beta * np.dot(pt_v, pt_norm) * pt_norm
                    current_pc.points[:2, i] = pt
