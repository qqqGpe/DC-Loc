...
double Ransac::computeModel() {
    ...
    int dim = p1.rows();
    int subset_size = 2;//choose 2 pairs of associated points
    int i = 0;
    for (i = 0; i < iterations; ++i) {
        ...
        Eigen::MatrixXd T_current; //p2 = T*p1 -> p2 = R*p1 + t
    ...
    vector<Vector3d> ps_train, ps_query;
    for(size_t i = 0; i < best_inliers.size(); i++){
        Vector3d p_1(p1(0,best_inliers[i]), p1(1,best_inliers[i]), 0);
        ps_query.push_back(p_1);
        Vector3d p_2(p2(0,best_inliers[i]), p2(1,best_inliers[i]), 0);
        ps_train.push_back(p_2);
    }
    Matrix4d trans;
    refineRegistration(ps_query,ps_train,trans);
    T_best = trans;
...
//    std::cout << "wbar is: " << wbar.transpose() << std::endl;
