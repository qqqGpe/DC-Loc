...
double cen2019descriptors(std::vector<double> azimuths, cv::Size polar_dims, Eigen::MatrixXd polar_points,
    ...
    cv::Mat polar_binary = cv::Mat::zeros(polar_dims.height, polar_dims.width, CV_32F);// 角度数量 * range bins
#pragma omp parallel for
    for (uint i = 0; i < polar_points.cols(); ++i) {
        polar_binary.at<float>(polar_points(0, i), polar_points(1, i)) = 1.0;//convert raw polar data to binary polar data
    }
    ...
    radar_polar_to_cartesian(azimuths, polar_binary, radar_resolution, cart_resolution, cart_pixel_width, true,
        cart_binary, CV_32F, navtech_version);// Convert keypoints to catesian binary points in catersian frame image
    ...
    convert_to_bev(cart_targets, cart_resolution, cart_pixel_width, bev_points);//Convert cartesian points center to middle position of the image

#pragma omp parallel for collapse(2)
    for (int i = 0; i < cart_binary.rows; ++i) {
        for (int j = 0; j < cart_binary.cols; ++j) {
            if (cart_binary.at<float>(i, j) > 0) { //对每一个特征点k都进行描述子的计算
...
#pragma omp atomic
                    d1.at<float>(k, azimuth_bin)++; //在特征点某一方向上探测到一个描述子时
#pragma omp atomic
